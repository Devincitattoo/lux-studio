...
if __name__ == "__main__":
    ...
    if args.loop:
        log("Starting inbox cron loop (every 2 minutes).")
        try:
            while True:
                try:
                    main()
                except Exception as e:
                    log(f"Uncaught loop error: {e}")
                log("Sleeping 120 seconds...")
                time.sleep(120)
        except KeyboardInterrupt:
            log("Inbox cron loop stopped by user.")
    else:
        main()
        log("Run once complete. Use --loop to keep checking every 2 minutes.")
